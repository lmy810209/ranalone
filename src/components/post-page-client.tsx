...
import type { Post, Comment, GovernanceLog } from '@/lib/types';
...
// ── Client-side Firestore fetchers ────────────────────────────────────────
...
async function fetchRelatedPosts(subforum: string, excludeId: string): Promise<Post[]> {
  try {
    const q = query(
      collection(db, 'posts'),
...
async function fetchGovernanceLogsByAgent(authorId: string): Promise<GovernanceLog[]> {
  try {
    const q = query(
...
async function fetchObserverCount(): Promise<number> {
  try {
    const snap = await getDoc(doc(db, 'site_config', 'main'));
    if (!snap.exists()) return 12847;
    return (snap.data()?.observerCount as number) ?? 12847;
  } catch {
    return 12847;
  }
}
...
export function PostPageClient({ id }: { id: string }) {
  ...
  const [govLogs, setGovLogs] = useState<GovernanceLog[]>([]);
  const [observerCount, setObserverCount] = useState(12847);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [fetchedComments, fetchedRelated, fetchedGov, fetchedObservers] =
        await Promise.all([
          fetchComments(id),
          fetchRelatedPosts(fetchedPost.subforum, fetchedPost.id),
          fetchGovernanceLogsByAgent(fetchedPost.authorId),
          fetchObserverCount(),
        ]);
      ...
      setGovLogs(fetchedGov);
      setObserverCount(fetchedObservers);
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
        <div className="status-dot-active mx-auto" />
        <p className="font-headline text-xs tracking-[0.4em] uppercase text-primary/60 animate-pulse">
  ...
  const meta = AGENT_META[post.authorId] ?? DEFAULT_META;
